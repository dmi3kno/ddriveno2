...
The authors start by factorizing the joint survival function $\bar{F}(x_1,x_2)$ as follows:

$$
\bar{F}(x_1, x_2)=P(X_1> x_1)P(X_2> x_2 \vert X_1 > x_1)= \bar{F}(x_1)\bar{F}_{21}(x_1,x_2)
$$

Recall, that the the joint survival function is $\bar{F}(x_1,x_2)=1-F_1(x_1)-F_2(x_2)+F(x_1,x_2)$ [@kundu2009BivariateGeneralizedExponential], which is a multivariate extension of the univariate survival function $\bar{F}(x)=1-F(x)$. 
...
So why do we care? Given what we have discussed in @perepolkin2021TenetsQuantilebasedInference, priors can be defined using the quantile functions without the need to define the densities. This not only means that you dont need PDFs to define the prior, but also that you don't need QDF, because it will cancel itself with the Jacobian.

Now with bQFs, you can define the bivariate priors using quantile functions, provided that the conditional bQF $Q_{21}$ makes sense.

Perhaps some Stan examples would be in order. I am sure we will come back to the bivariate quantiles for some cool quantile distribution soon.
